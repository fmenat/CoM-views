...
def possible_missing_mask(n_views):
    #all possible combinations of missing views
...
def create_list_options(n_views):
    ...
    return list(itertools.product([0, 1], repeat=n_views))[1:][::-1] #from all views to views to missing
...
def augment_all_missing(view_names_forward):
    ...
    for i in range(n_missing_mask):
        views_mask_i = np.asarray(lst_missing[i]).astype(bool) 
